"""ClearView adviser portal extraction module."""
...
POLICY_LIST_URL = "https://adviser.clearview.com.au/policies"
POLICY_ROW_SELECTOR = ".policy-table-row"
NEXT_PAGE_SELECTOR = ".pagination .next:not(.disabled)"


class ClearviewExtractor(BasePortalExtractor):
    async def extract(self, context: BrowserContext) -> list[dict]:
        ...
        while True:
            ...
            for row in rows:
                cells = await row.query_selector_all("td, .cell-value")
                if len(cells) < 7:
                    continue
        ...
        log.info("ClearView extraction complete — %d policies", len(policies))
        return policies
